...
class TestData(Dataset):
    def __init__(self, testpath, iscrop=True, crop_size=224, scale=1.25, face_detector='fan', sample_step=10, use_mica=False):
        '''
    ...
    def __getitem__(self, index):
        ...
        return {'image': torch.tensor(dst_image).float(),
                ...
                'arcface_inp': arcface_inp
                }
    


class TestData_stylegan(Dataset):
    def __init__(self, stylegan2_gen, batch_size=4, iscrop=True, crop_size=224, scale=1.25, face_detector='fan', sample_step=10, use_mica=False):
        ...
        self.stylegan2_gen = stylegan2_gen
        self.use_mica = use_mica
    ...
    def __getitem__(self, img_gt):
        # imagepath = self.imagepath_list[index]
        # imagename = os.path.splitext(os.path.split(imagepath)[-1])[0]
        # image = np.array(imread(imagepath))
        # sample_z = torch.randn(self.batch_size, 512, device='cuda')
        # image = self.stylegan2_gen([sample_z])[0]
        image = img_gt.permute(1,2,0).detach().cpu().numpy()
