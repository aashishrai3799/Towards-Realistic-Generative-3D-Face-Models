...
from decalib.datasets.celebA import CelebAHQDataset
...
def build_train(config, is_train=True):
    ...
    if 'celebahq' in config.training_data:
        data_list.append(CelebAHQDataset(image_size=config.image_size, scale=[config.scale_min, config.scale_max], data_dir=config.celebA_path, trans_scale=config.trans_scale))
