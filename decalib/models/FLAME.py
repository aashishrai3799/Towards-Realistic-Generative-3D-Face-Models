...
class FLAME(nn.Module):
    ...
    def __init__(self, config, optimize_basis=False):
        ...
        shapedirs = torch.cat([shapedirs[:, :, :self.n_shape], shapedirs[:, :, 300:300 + config.n_exp]], 2)
    ...
    def project_to_shape_basis(self, shape_vector, shape_as_offset=False):
        ...
        return torch.matmul(diff.reshape(batch_size, -1),
                            self.shapedirs[:, :, :n_eigenvectors].reshape(3 * n_vertices, n_eigenvectors))
    ...
    def compute_distance_to_basis(self, shape_vector, shape_as_offset=False):
        ...
        if shape_as_offset:
            ...
            diff = shape_vector - self.v_template
        shape_params = torch.matmul(diff.reshape(batch_size, -1),
                                    self.shapedirs[:, :, :n_eigenvectors].reshape(3 * n_vertices, n_eigenvectors))
        distance = diff - torch.matmul(shape_params, self.shapedirs[:, :, :n_eigenvectors].reshape(n_vertices * 3,
                                                                                                   n_eigenvectors).t()).reshape(
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None
        , neck_pose_params=None, shape_basis_delta=None):
        """
            Input:
        ...
        if expression_params is None:
            expression_params = torch.zeros([1, 100], dtype=self.dtype, requires_grad=False,
                                            device=self.neck_pose.device).expand(batch_size, -1)

        if shape_params.shape[1] != self.n_shape:  # For MICA this condition won't hold true
            # Adding 200 zeros to end of DECA code to support 300 shape params
            num_additional_zeros = self.n_shape - shape_params.shape[1]
            shape_params = torch.cat([shape_params, torch.zeros((batch_size, num_additional_zeros),
                device=shape_params.device)], dim=1)
